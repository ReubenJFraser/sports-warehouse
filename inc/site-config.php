...
foreach ($demographics as $demo) {
  $slides = [];
  $videos = []; // associative by videoKey
  foreach ($raw as $brandKey => $conf) {
    ...
    foreach ($conf['slides'] as $slide) {
      if (stripos($slide['banner'], "/{$demo}/") !== false) {
        ...
        if (!empty($slide['videoKey']) && isset($conf['videos'][$slide['videoKey']])) {
  ...
  $raw[$demo] = [
    'slides' => $slides,
    // keep associative mapping so hero.php can find videos via $config[$page]['videos'][$videoKey]
    'videos' => $videos,
  ];
}
