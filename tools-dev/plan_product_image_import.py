...
def write_summary(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    lines = [
        ...
        "",
        f"- Total ProductDB rows considered: {len(rows)}",
        f"- Total high-confidence copy candidates: {action_counts.get('copy', 0)}",
        f"- Total manual_review rows: {action_counts.get('manual_review', 0)}",
        f"- Total skipped rows: {action_counts.get('skip', 0)}",
        "",
        "## Rows by action",
    ]
    ...
    lines.append(f"- Missing ProductDB images paths: {len(missing_images)}")
    lines.append(f"- Matched folders with no source files: {len(no_source_files)}")
